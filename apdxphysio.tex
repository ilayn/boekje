...
terminology is yet to reach a steady-state standard, however, what follows below seems plausible considering the variations and nuances 
found in the literature. Since there is no fixed definition for such perception we would use a rough  classification based on the magnitude-%
...
There is also another process, called \emph{neural} or \emph{sensorial adaptation} which quantifies the frequency decay of firing under 
constant stimulus. We can see the tangible effect of this process frequently e.g., our nose looses the sensitivity to a powerful smell if 
exposed to it for some time or we stop noticing the touch of glasses on the face or the ring on the finger. Some sensors have a slow decay 
