...
%
%
%In other words, if we aim for the ultimate goal directly then there is no 
%need to consider the delay problem since we can expect that the communication technology would be advanced enough to circumvent these 
%problems at the time we obtain perfect bilateral teleoperation in the undelayed case. Unfortunately, we are not even close to perfection
%in the undelayed case. 
%
%
%
%As we mentioned before, transparency
%and other criteria might still be true as they are to ideal teleoperation as live performance feeling is to audio. 
...
%
%Finally, based on our control design attempts given in \Cref{chap:application}, it seems that all performance objectives above 
%describe a particular projection of a more general but yet unknown objective in terms of human perception. 
%
%
%mphasizing one performance objective to the other one can obtain different controllers
