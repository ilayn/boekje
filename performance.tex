...
Second option is to make our system behave like an ideal transparent system as much as possible. If we use the notation
$N$ for the overall controlled teleoperation system model with the controller $K$, we can use a performance index
\[
\min_K \abs{\pmatr{f_{human}-f_{remote}\\f_{env}-f_{local}\\x_{local}-x_{remote}\\\vdots}} 
\]
