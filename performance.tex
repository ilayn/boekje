...
this performance index. Moreover, it's not easy to search for such $K$ since the cost function to be minimized is not 
guaranteed to be continuous or smooth etc. As we have observed in the literature, almost every transparency optimized
control selects the full performance first hence assuming the global minimizer and then tries to stabilize the system 
...
not to be living on the boundary of the uncertainty set. Therefore, either by gridding the uncertainty set, the stability 
guarantee has to be obtained via testing the conditions on a high number of points or a specific relaxation on the constraints 
need to be imposed such that testing the conditions on finitely many (and computationally tractable) number of points implies 
stability over the whole uncertainty set.
...
Note that the matrix case follows a similar but more involved computation moreover one can recognize the familier
plant-uncertainty representaion clearer without any complication. This led to the authors of \cite{cavusoglu} 
