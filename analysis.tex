...
hence
\[
\operatorname{im}\pmatr{I\\G} \cap \operatorname{im}\pmatr{\Delta \\ I} =\{\mathbb{0}\} ,\ \forall \omega\in\Realext.
\]
...
\subsection{Llewellyn's Stability Criteria}\label{sec:llewellyn}
...
The stability conditions of \Cref{thm:llw} can be reproduced via the IQC theorem as follows. If $\Delta_l$ and $\Delta_s$ are passive and stable LTI systems, they satisfy
\[
...
points in $[0,10000] \si[per-mode=fraction]{\radian\per\second}$ and solve, at each grid point, 
...
$Z_e = \frac{k}{s}$ with an uncertain constant stiffness coefficient $k\in[0,\bar{k}]$ 
