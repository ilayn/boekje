...
that are made famous by the $\mu-$syntesis tool $D-K$ iteration. Also, one can get a rough 
...
plane different then the whole right half plane as long as it can be represented (or covered with) with a 
quadratic constraint. Thus, once this multiplier type is set, it's a matter of evaluating the respective FDI for the system
...
the stability condition can be posed as the existence of $\lambda_1(\omega),\lambda_2(\omega)>0$ for all $\omega\in\Real_e$ such that
\begin{equation}
\pmatr{1 &0\\0 &1\\\hline-N_{11} &-N_{12}\\-N_{21} &-N_{22}}^*
\left(\begin{array}{cc|cc}
      0 &0                   &1 &0                  \\
...
contrary to the claim, those results can only be sufficient since their test only deals with the first part of the constraint, namely 
...
uncertainties without invoking the passivity theorem in a few steps instead of deriving explicit standalone conditions for each individual case.
