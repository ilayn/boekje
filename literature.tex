...
i.e. each time the lever arm pulls back a thread of the gear at each time and the cycle repeats the resulting relative motion is very much 
...
above is, again invoking the analogy, based on the closed clutch response of the arm. In other words, what is measured is a cumulative 
spring-like behavior of the arm that is actively controlled to be kept at a certain configuration. This is related to the \enquote{human's 
restoring force} given in \Cref{rem:filament}. 

Having a spring-like property naturally implies that the arm is passive. But the problem with this argument, as far as 
the author understands, is that there is no reason to assume that the human arm exhibits a symmetric positive definite 
...
and many more which we will omit here. These studies also involve model-based control with varying dependence on 
the model.
...
controllers are to the best of our knowledge not pursued to the extent the theory allows for yet.

Moreover, the proposed methods often avoid the discussion on how to tune the PID controllers for the local, remote,
...
the derivations. This is yet another reason for why we have chosen IQC framework for robustness analysis and model-based control
...
nonlinear methods) the covered part here is also a considerably large part. In the next chapter they are detailed 
...
survey of \cite{klomp} which provides also a comprehensive outlook to the teleoperation literature in a very detailed 
