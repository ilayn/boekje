...
%subjective nature involving the human liking. However, to put it bluntly, the experts are not helping either. In 
%other words, most of the \enquote{good performance} motivations come from the first principles, Newton's laws of 
%motion, conservation of energy (but surprisingly avoiding conservation of momentum) etc. Some literature argue
%that since most of the tools we, human operators, utilize are (almost-)lossless, say, a screwdriver or even a simple
%stick, it's natural to seek for a passive bilateral teleoperation system that ideally behaves like a rigid
%transmission mechanism. 
%
%Therefore, using a vision analogy, the better a system transmits the remote motion to the local site, the better
...
%for the ideal case. Then the resulting controller is denoted with \enquote{\emph{Transparency Optimized Controller}}. 
%It should be clear that this is the transparency definition form the passivity point of view and we refrain from iterating
%what is given above. 
%
%Even so, we have to emphasize that we have not touched the most important question, that is the first of the three, rather 
%we hope to achieve the required transparency levels just enough to fool the user. After so much time, this point is in our 
%opinion simply discarded and many studies in the literature somewhat treats the conclusions of Lawrence in a different 
