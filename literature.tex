...
far from comprehensive and deliverately shaped with pragmatic intentions. Hence a large body of research is 
...
\caption{A transparent two-port network with passive terminations. (Source: Fred Dean Jr., \href{http://www.flickr.com/photos/be216cd1/5971825854/}{[Flickr:Fred Dean Jnr]})}%
\label{fig:lit:train}%
\end{figure}
...
If moreover,  human, and the environment models are LTI then absoulte stability theorem is an exact stability characterization. 
...
of the $D$-scalings in the $\mu$-tools. Thanks to the frequency domain formulation, it also allows us to rewrite the condition 
...
together with \eqref{eq:lit:llew2}. 
...
small-gain theorem requires a normed space whereas passivity theorem requires an inner product space, hence the applicability is relatively limited. 

As we will show in \Cref{chap:analysis}, one can reproduce the results of Llewellyn and also the passivity theorem using the IQC framework. 
...
\begin{equation}
s:[0,\infty]\times[\infty,\infty] \mapsto \left\{ z\in\Complex \mid \abs{z} \leq 1 \right\}\quad, \quad s(z) = \frac{z-1}{z+1}
\label{eq:lit:smith}
\end{equation}
...
\end{scope}
\end{tikzpicture}
\caption{Mapping the closed right half plane onto the unit disc. Origin is shown with a white dot.}%
\label{fig:lit:smith}%
\end{figure}

Under this mapping the Nyquist curve of $e^{-sT}$ (unit circle) is mapped onto the imaginary axis, 
hence unbounded. This once agin shows that delay uncertainty does not satisfy the norm constraint
$\|s(e^{-\iw t})\|_\infty \leq 1$ to invoke the small-gain theorem in the transformed coordinates. 
Had it been the case that the uncertainty was bounded by one, then it would have been possible to 
conclude stability. Thus, these transformations are not beneficial for analysis, however, following 
the cue from the previous mapping results, papers \cite{andersonspong} and \cite{nieslotine} made it 
possible to design controllers that takes into account this delay uncertainty. The resulting loop 
is stable regardless of delay period, hence they belong to the class of methods often distinguished
by ``delay-independent'' methods. According to the literature, these are the most common 

 
The major disadvantage of the passivity methods are that
...
\subsection{2-,3-, and 4-Channel Control}
