...
The experiments are performed with the same device at one shot with an arbitrary combination of motion profiles listed above. Due to the
...
literature}, it is also an unrealistic goal at the outset. As we will show the problem is not essentially a state-constraint problem but 
rather the inherent stiffness modes of the devices that are excited in the rigid contact case. 

\subsection{Full Experiment Motion}

In each controller test, a right-handed male operator tried out the different scenarios listed above in about $1$-$2$ minute. The resulting overall
datastream is shown in \Cref{fig:app:design1overall,fig:app:design2overall,fig:app:design3overall}. One can 

\begin{figure}%
\centering
\begin{tikzpicture}
	\begin{axis}[width=.75\textwidth,height=5cm,xlabel=Time,ylabel={Position},unit code/.code 2 args={\si{#1#2}},x unit=\second, y unit=\degree]
        \addplot+[ultra thin,no marks] table[x index =0,y expr={-deg(\thisrowno{1})}] {plotdata/overalld2.dat};
        \addplot+[ultra thin,no marks] table[x index =0,y expr={-deg(\thisrowno{2})}] {plotdata/overalld2.dat};
    \end{axis}
%\draw[-stealth] node[append after command={(a) -- (2,3.5)}] (a) at (-1,3) {Free Air};
\end{tikzpicture}
\caption{Design 1 with arbitrary fast varying stiffness parameters and fixed mass and damping coefficient.}%
...
\begin{tikzpicture}
	\begin{axis}[width=.75\textwidth,height=5cm,xlabel=Time,ylabel={Position},unit code/.code 2 args={\si{#1#2}},x unit=\second, y unit=\degree]
        \addplot+[ultra thin,no marks] table[x index =0,y expr={-deg(\thisrowno{1})}] {plotdata/overalld1.dat};
        \addplot+[ultra thin,no marks] table[x index =0,y expr={-deg(\thisrowno{2})}] {plotdata/overalld1.dat};
    \end{axis}
%\draw[-stealth] node[append after command={(a) -- (2,3.5)}] (a) at (-1,3) {Free Air};
\end{tikzpicture}
\caption{Design 2 with slowly varying human model stiffness parameter and fixed mass and damping coefficient.}%
...
\begin{tikzpicture}
	\begin{axis}[width=.75\textwidth,height=5cm,xlabel=Time,ylabel={Position},unit code/.code 2 args={\si{#1#2}},x unit=\second, y unit=\degree]
        \addplot+[ultra thin,no marks] table[x index =0,y expr={-deg(\thisrowno{1})}] {plotdata/overalld0.dat};
        \addplot+[ultra thin,no marks] table[x index =0,y expr={-deg(\thisrowno{2})}] {plotdata/overalld0.dat};
    \end{axis}
%\draw[-stealth] node[append after command={(a) -- (2,3.5)}] (a) at (-1,3) {Free Air};
\end{tikzpicture}
\caption{Design 3 with slowly varying human model parameters.}%
\label{fig:app:design3overall}%
\end{figure}

due to the fact that a rigid contact with a relatively tighter grip clamps the device. 
