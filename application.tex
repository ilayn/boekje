...
We nevertheless present the full model LFT interconnection structure but in the controller design case only the simplified 
model is used. 
...
\node[draw] (mas) at (0,0) {$\displaystyle \frac{1}{m_ms^2+b_ms+k_m}$};
...
cases will be using fixed mass and damping coefficients in the \Cref{fig:app:design1}. 
...
\Cref{fig:tab:design1}. The maximum and the minimum singular values of the controller are shown in  \Cref{fig:app:ksigma1}.
Notice that, even though the iterations lead to full uncertainty case $r=1$, due to the performance mismatch the iteration 
is continued until an agreement up to some precision is achieved. 
...
Using the multiplier structure given in \Cref{sec:tvrovsec}, with basis function 
\[
\Phi(s) = \pmatr{1\\\frac{1}{s+1}}
\]
we utilize the following multiplier structure
...
\left(\begin{array}{cc|c|cc|c}
    ...
    \right)
\]
we obtain lower worst case $\mathcal{L}_2$ gains as shown in the \Cref{fig:tab:design2d1}. To demonstrate the conservatism 
reduction via increasing the basis length in the IQC multiplier parametrization we use the basis length $d=2$, i.e.,
\[
...
significant changes hence even with the relatively short basis lengths we arrive at improved results. The improvements are not 
only in terms of the resulting worst case $\mathcal{L}_2$ gains which tell very little if not any about the real-time implementation 
performance. We show that the improvement is also visible in the experiments shown in the next section. 
...
together with arbitrarily fast varying environment stiffness coefficient uncertainty. 
