\chapter{Controller Design for the TU/e 1-DoF System}
...
\node[draw] (mas) at (0,0) {Slave Device};
\node[circle,draw,inner sep=2pt] (sum) at ([xshift=6mm]mas.east) {};
\draw[->] (op) -| (sum);
\draw[<-] (mas) -- (sum);
\draw[->] ([yshift=-5mm]sum.south) node[below](fm){$f_s$}-- (sum);
\draw[->] (op-|sum) -- ++(0,5mm)node[left] {$f_e$};
\draw[->] (mas.west) -| ++(-10mm,5mm) |- (op);
\node (xm) at ([xshift=-10mm]mas.west|-fm) {$x_s$};
\draw[->] (mas) -|(xm);
...
$C_1$: \tikz[baseline]\draw[thick,dotted] (0,0)--++(5mm,0);.}%
...
Still, in accordance with our claims in the \Cref{chap:perf}, this design, regardless how impressive it is, cannot be accepted as a genuine 
...
passivity and simultaneously achieving precise tracking} or some controller entries in \Cref{lit:syn:ela} are assumed to be zero. In the 
