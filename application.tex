...
here with \enquote{dragging}. This happens when the same controller tries to follow both objectives at once with the same controller only penalizing the 
difference of the positions and forces separately or in the passivity case only allowing the motion when it is safe to move in terms of energy input. 
To put the reason why a MIMO controller avoids this problem into context,  consider \Cref{fig:app:design1forceoverall,fig:app:design1freeair}. We can 
see that the measured human force and environment forces are different since the remote device is in free-air and the human is actually applying force 
on the local device. The environment force recording is relatively smaller but nonzero due to the inertia and the friction of the remote device. 

It should be noted that even the human is applying a nonnegligible force profile, the controllers are in fact trying to close this force mismatch 
via applying a torque along the human motion i.e. acting like a friction compensator thus reducing the difference $f_h-f_l$. Similarly in the remote 
device the difference is increased via $f_e-f_l$ via tracking closely the local force vector. These combined leads to the very good position tracking
and as a bonus we don't need a precise force tracking until the positions of the devices are constrained or distorted which is presented below.
...
One can see that the tracking property of $C_2$ superior to the other two though the
...
The hard contact measurements are given in \Cref{fig:app:design1hard,fig:app:design2hard,fig:app:design3hard}. In this case
the immediate difference compared to the previous cases is that $C_3$ starts to render the hard spring softer hence the increase in
...
model is surely dominated with such numbers. Therefore, though it is not guaranteed, the numerical algorithm is somehow forced to 
include some robustness around this comparatively huge number. Consequently, after a threshold value the uncertainty limit 
$2\cdot 10^5$ is qualitatively not that different than $1\cdot 10^6$ in terms of robustness properties. Hence, it is not completely
true that there is no robustness constraint imposed on the system. 
...
As seen around \SI{84}{\second} and at the end the device show temporarily an oscillatory behavior. In the end of the motion, $C_3$ 
...
It is quite the contrary for $C_1$ and $C_2$, the rigid contact performance is quite satisfactory. We can distinguish the effect of 
taking the time varying uncertainty variation bound into account as $C_1$ shows superior performance even with a few contact sliding
motion after pressing down the rigid block. Moreover towards the end we can see the contact tapping responses.  


Although not visible in the plots, $C_1$,$C_2$ suffers from an effect due to the assumptions we have made about the human mass coefficient.
...
are tuned for continous contact but the the human touches just enough to register a nonnegligable force on the sensor without actually
...
higher velocity on to the contact\footnote{As the reader might notice, we have realized later that the immersion levels tend to fool 
the users use phrases as if there is an actual hammering action taking place.} 
