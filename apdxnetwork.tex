...
or
\begin{equation*}2R_{11}R_{22}-\abs{N_{12}N_{21}}-\Re{N_{12}N_{21}} > 0\tag{\ref{eq:lit:llew3}$'$}\end{equation*}%\label{eq:lit:llew3p}
for all $\omega\in\Realext$, where $R_{ij}$ and $X_{ij}$ denote the real and imaginary parts of $N_{ij}$ respectively.
\end{thm}

When the network realization is given in scattering parameters, Llewellyn's conditions are known as Rollett's conditions (\cite{stern,rollett,kurokawa}).
Then the absolute stability conditions are formulated as follows: 
